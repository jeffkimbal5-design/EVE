...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    addLog('info', 'Booting EVE PRIME Neurosymbolic Kernel v3.0...');
    addLog('agent', 'System 1 (Neural Intuition) and System 2 (Symbolic Rigor Core) online.');
    fetchSystemStatus()
      .then((status) => {
        setSystemStatus(status);
        addLog(
          'agent',
          `Kernel initialized. Active Model: ${status.activeModel}. Verification Prover: ACTIVE. Auth: ${
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-cyan-500/20 selection:text-cyan-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        {activeView === 'hypercube' ? (
          /* 4D Neurosymbolic Hypercube Matrix & Runic Cipher Engine */
          <HypercubeStudio
            onLogEvent={addLog}
            speak={speak}
          />
        ) : activeView === 'neurosymbolic' ? (
          /* Neurosymbolic Architecture & Self-Modifying Studio */
          <SelfModifyingStudio
            onLogEvent={addLog}
            speak={speak}
          />
      ...
      <footer className="border-t border-slate-900 bg-slate-950/80 py-4 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500 font-mono">
          <div>
            EVE_PRIME Neurosymbolic Engine &copy; 2026 Jeffrey L. Kimbal. MIT License.
          </div>
          <div className="flex items-center gap-3">
            <span>Dual-System: Gemini 3.8 Flash + Symbolic FOL Prover</span>
            <span>|</span>
